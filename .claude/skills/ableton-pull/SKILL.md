...
- "everything" → run every available domain in order: `mix-state`, then `cue-points`, then `devices`, then `device-parameters`, then `arrangement-clips`, then `session-clips`, then `clip-notes`, then `envelopes`. (`score-globals` is a subset of `mix-state`'s probes; skip it.)

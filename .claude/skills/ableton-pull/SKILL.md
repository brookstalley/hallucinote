...
- "everything" → run every available domain in order: `mix-state`, then `cue-points`, then `devices`, then `arrangement-clips`, then `session-clips`, then `clip-notes`. (`score-globals` is a subset of `mix-state`'s probes; skip it.)

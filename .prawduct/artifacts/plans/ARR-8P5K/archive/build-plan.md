...
- [x] Chunk 1 — Taxonomy-coherence delta to `arrangement-model.md` (D1–D4 + coherence
      verification + backlog updates) — DONE on `docs/arr-8p5k-taxonomy-coherence`.
      D1 (meter-feel split + non-promotion + trigger + ARR-4M3T xref), D2 (energy→ARR-7M3D
      #139, recurrence→ARR-9K4T #137 MEASURE-half status), D3 (line-vs-arrangement
      motivic-economy boundary, canonical in the doc's own text) applied; D4 = no-op
      (meter not in the NOT-modeled list — confirmed). Backlog: ARR-8P5K + ARR-4M3T updated;
      no item closed (umbrella stays open as the recurring coherence guard).

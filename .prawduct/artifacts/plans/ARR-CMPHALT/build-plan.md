...
- **Description:** Make the comparator tolerance-stable at bucket boundaries so a note present on both sides (equal pitch, starts within `eps`, matching dur/vel post-normalization) can **never** appear in both `missing` and `extra`. The plan offered two candidate mechanisms (a flat `missing`↔`extra` post-pass, or greedy nearest-neighbor key pairing), with the mechanism delegated to build time per Open assumption (b). **Chosen mechanism (as built):** a *key-level* reconcile — keep the bucket grouping, pair exact `(pitch, bucket)` keys first, then reconcile leftover db-only/live-only keys that are same-pitch, adjacent-bucket, and within `eps` on their representative starts; a reconciled pair flows through the **existing stack-matches-ANY-member** check. **Why the key-level form over the flat post-pass:** (1) it composes with the same-(pitch,start) wildness-stack collapse — a stack that *also* straddles a boundary reconciles against the whole stack, not just the first representative the flat post-pass would see, so it can't false-mismatch; (2) a boundary twin whose dur/vel has genuinely *drifted* reclassifies to a single `mismatch` rather than a spurious `missing`+`extra` pair (marginally safer for the detector). Did not change `eps`/`vel_tol` or the collapse semantics.

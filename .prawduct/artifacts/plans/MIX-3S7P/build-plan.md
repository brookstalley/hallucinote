...
- [ ] Chunk 1 — plan-level host-clip resolution test (test-created session + links; all 6
      intro+break envelopes emit, asserted by identity, no "no coverage" skip)
- [ ] Chunk 2 — render-level send verification (ENTRY-GATED on fresh correctly-keyed capture)
      + surface by-ear amounts (Live UP)
- [ ] Chunk 3 — record decision under governance + reconcile backlog (doc-only, final)

**Context:** MIX-3S7P is substantially ALREADY SHIPPED + test-locked in `hallucinote-songs`
(atmosphere clip-local pan/send lanes, `decisions/08` §5,
`test_atmosphere_envelopes_are_clip_local`). This plan VERIFIES + RECORDS rather than
re-authoring. Genuinely open: the song-spanning DubDelay dynamic send (user-deferred to
post-v1 — PENDING user fork, NOT a chunk) and the exact by-ear amounts (pan width +
aesthetic + boundary snap — surfaced, never guessed; Live unattended). Next: Chunk 1.

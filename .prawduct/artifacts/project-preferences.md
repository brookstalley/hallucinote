...
- **Write what a thing IS. Never define it by what it isn't.** (Ratified 2026-08-11; retroactive — apply when touching existing prose.) Two banned moves: **positioning against alternatives** ("unlike X", "what makes this different from Y", "everyone else's Z is a closed binary") and **rebutting an objection the reader hasn't raised** (a `## What it isn't` section, an FAQ entry answering a critic). State the capability, the mechanism, the fact; the negative space is implicit and the reader draws it themselves. *Why:* defining by contrast argues with someone who isn't in the room, which reads as defensive and plants the doubt it set out to answer — and it dates the doc to whatever it was reacting to.

  **What this does NOT ban**, because the first draft of this row did and the corpus immediately violated it:
  - **A factual distinction that clarifies a mechanism or sets an expectation** — "a long agentic workflow, not a chat", "the song is reproducible; the act of composing it isn't", "ten minutes buys a playing song, not a finished one". Those inform; they don't defend.
  - **Naming prior art as lineage** — "plain-text music has a long line behind it: TidalCycles, Sonic Pi, Lilypond, DAWproject". Naming other tools is fine; ranking yourself against them is the banned move.
  - **A limitations register** (`docs/known-issues.md`, a `## Non-goals` section for contributors), which states plain facts rather than defence.

  The test is what the sentence is *doing*: claiming superiority or difference over another tool, or answering a critic (banned), versus describing how this thing works, what it costs, or where it sits (fine).
...
| Prose says what a thing IS; never defines it by what it isn't (§ Documentation & prose) | Critic | Goal 4 — judgment-required; the tells are a `## What it isn't` heading, "unlike X", "what makes this different from Y", and an FAQ entry answering an objection nobody raised. The test is competitor/critic (banned) vs mechanism, cost or where-it-sits (fine) — a factual distinction like "a long agentic workflow, not a chat" passes, and so does naming prior art as lineage. Scope note: the owner's personal `brooks-writing-style.md` (outside this repo) bans the narrower "not just X, it's Y" reframe; this row is the general rule, binding here, and the two agree where they overlap | janitor | Defining by contrast argues with someone who isn't in the room, which plants the doubt it answers and dates the doc to whatever it was reacting to. Ratified 2026-08-11 after an intermediate draft of the launch-readiness pass shipped exactly that mistake — and narrowed the same day, when the first wording proved to outlaw ordinary factual distinctions the corpus needs |

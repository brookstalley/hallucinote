...
- **Write what a thing IS. Never define it by what it isn't.** (Ratified 2026-08-11; retroactive — apply when touching existing prose.) Two banned moves: **positioning against alternatives** ("unlike X", "what makes this different from Y", "everyone else's Z is a closed binary") and **rebutting an objection the reader hasn't raised** (a `## What it isn't` section, an FAQ entry answering a critic). State the capability, the mechanism, the fact; the negative space is implicit and the reader draws it themselves. *Why:* defining by contrast argues with someone who isn't in the room, which reads as defensive and plants the doubt it set out to answer — and it dates the doc to whatever it was reacting to.
  ...
  **What this does NOT ban**, because the first draft of this row did and the corpus immediately violated it: an ordinary factual distinction that clarifies a mechanism or sets an expectation — "a long agentic workflow, not a chat", "the song is reproducible; the act of composing it isn't", "ten minutes buys a playing song, not a finished one". Those inform; they don't defend. The test is whether the sentence is *about a competitor or a critic* (banned) or *about how this thing works* (fine). A limitations register (`docs/known-issues.md`, a `## Non-goals` section for contributors) is likewise fine, and states plain facts rather than defence.

...
- **Test location**: three categories, intentionally separated.
  ...
  - **Song-specific** — `<workspace>/<slug>/tests/test_*.py` lives alongside that song's `build.py` and `captured_session.json` so each song is a self-contained unit (per `docs/VISION.md` "songs as git repos"). These tests assert song-level shape (track count, a floor on note count, the expected sections in order) and may load that song's snapshot. The exact thresholds live in the test files — see them for current contract values. These tests never test platform behavior incidentally. **They do not live in this repo:** authored songs live in their own workspace repo (`hallucinote-songs`) with its own pytest config. The one in-repo workspace is the `examples/` bounded exception below.
  - Pytest discovers the framework's own two categories via `[tool.pytest.ini_options].testpaths`; the authoritative list is in `pyproject.toml`, not restated here. It also includes `examples/` — the in-repo demo workspace (`examples/hallucinote.toml`, `layout = "monorepo"`, `songs_root = "."`), so the demo song's tests join the default run the moment one lands. Song test dirs intentionally have no `__init__.py` (rootdir-discovered) so they don't collide with the platform `tests/` package namespace.
...
- **Error handling**: Plain exceptions (`ValueError`, `KeyError`). No custom exception hierarchy. Never swallow exceptions silently — waive a genuinely necessary broad catch with `# prawduct:allow prawduct/broad-except -- <reason>`, stating the reason. This is the only spelling: the legacy `# prawduct:ok-broad-except` form was migrated out of the tree in full (2026-08-11), so a `grep` for it returning anything is a regression.
...
- **File organization**: Layer folders inside `src/hallucinote/`: `db/` (state + events + mutators + queries), `generators/` (pure musical building blocks), `sync/` (DB↔Ableton bridge). Song-specific builders live under `<workspace>/<slug>/build.py` — in the songs workspace repo, not here — and consume the library.
...
- **Dev commands**:
  ...
  - `python <workspace>/<slug>/build.py [--reset]` — build a song into its SQLite DB. Run it from the songs workspace; no song in this repo is a valid target (`examples/` is the one in-repo workspace and currently holds no song).
- **DB files (prescriptive)**: **one SQLite DB per song**, in that song's own directory inside its workspace. The filename is per-branch — `<slug>-<branch>.db`, falling back to `<slug>.db` outside a repo or on detached HEAD (`resolve_db_path` is the single source of truth; don't hand-build the path). The directory name, the DB filename stem, and `songs.name` (the slug) must all match. The slug is filesystem-safe: `[a-z0-9_-]+` (lowercase, digits, hyphens, underscores). Human-facing names with spaces / capitals / punctuation go in `songs.title`. No sidecar config files; the DB schema is the source of truth for song metadata. `*.db` is gitignored.
...
| One DB per song, in the song's own directory in its workspace, named by `resolve_db_path`; slug = `[a-z0-9_-]+`; display name in `songs.title` | Test + Critic | Schema `CHECK` on `songs.name` + Python regex in `create_song` enforce the slug; Critic Goal 4 catches setup violations (rogue paths, sidecar config files) | janitor | A song is forkable only if its directory is self-contained and its name is filesystem-safe on every platform |

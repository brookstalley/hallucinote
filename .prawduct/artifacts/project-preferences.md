...
- **Write what a thing IS. Never define it by what it isn't.** (Ratified 2026-08-11; **absolute reading restored by owner ruling 2026-08-12**; retroactive — apply when touching existing prose.) State the capability, the mechanism, the fact; the negative space is implicit and the reader draws it themselves. *Why:* defining by contrast argues with someone who isn't in the room, which reads as defensive and plants the doubt it set out to answer — and it dates the doc to whatever it was reacting to.
  ...
  **Permitted, owner-ratified:**
  ...
  - **A limitations register** (`docs/known-issues.md`, a `## Non-goals` section for contributors), which states plain facts rather than defence. (In the owner's original 2026-08-11 wording as "Narrow exception" — it did **not** arrive with the rejected narrowing.)

  **Permitted in practice, PENDING OWNER RATIFICATION** — applied by the 2026-08-12 sweep, recorded here rather than quietly folded into the rule. If the owner vetoes either, the six in-scope files need another pass:
  - **A prescriptive rule may prohibit.** "Write tests alongside the code, never after" is an instruction, not a description of a thing. The norm governs prose that *describes*.
  - **Verbatim quotation is untouchable.** `docs/tour.md` quotes session transcript, the user's own words, and code from `build.py`. Rewriting captured evidence to satisfy a prose norm would falsify the thing the page exists to show. This one should be ratified on principle even if the first is not.

  **Scope — positioning prose** (owner-set 2026-08-12): `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `docs/VISION.md`, `docs/faq.md`, `docs/tour.md`. Reference docs elsewhere under `docs/`, agent-read `skills/`, `.prawduct/artifacts/` and the frozen `docs/archive/` are outside it — in operating instructions the construction disambiguates ("the list itself, not the wrapper") rather than defends, and precise agent instructions are the product. Widening the scope is the owner's call, not a sweep's.

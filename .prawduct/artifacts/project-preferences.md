...
| Prose says what a thing IS, never what it isn't (§ Documentation & prose) | Critic | Goal 4 — judgment-required; the tells are a `## What it isn't` heading, "unlike X", "this isn't a Y", and an FAQ entry answering an objection nobody raised. Scope note: `brooks-writing-style.md` bans the narrower "not just X, it's Y" reframe — this row is the general rule and the two agree where they overlap | janitor | Defining by contrast argues with a critic the reader hasn't met, which plants the doubt it answers and dates the doc to whatever it was reacting to. Ratified 2026-08-11 after an intermediate draft of the launch-readiness pass shipped exactly that mistake |

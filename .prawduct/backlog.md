...
- **DB schema for declared `reverb_send_intent` (RT60 per send).** Chunk 3 ships `verify_reverb_send()` working on caller-supplied `DeclaredReverbSend` records, but the song DB has no schema yet for declaring "this send is intended to be RT60 = X". Until that lands, real-song `analyze_mix` invocations emit a structured `skipped_analyses` record explaining the gap (handler-level teaching). Schema-shape sketch: per-`sends` row (existing) → add `intended_rt60_s REAL NULL`; mutator + query; the analysis handler reads these rows and assembles `DeclaredReverbSend` records before calling `analyze_mix`. **Verifiable signal:** `songs/<slug>/<slug>.db` carries declared RT60s queryable through `hallucinote.db.queries` AND `ableton_analysis(action='analyze')` populates `reverb_verifications` from those declarations without caller-supplied sends. **Sized:** small. (audio-analysis MVP Chunk 3 close-out 2026-05-28)

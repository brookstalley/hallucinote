...
- **Audio analysis (`ableton_analysis`) — the offline analysis pipeline atop captures.** Audio-analysis MVP Chunk 2 closed 2026-05-27 with `ableton_render` shipping multi-analyzer simultaneous capture (PDC peak lag 0 samples). `ableton_analysis` is the remaining half — consume the captures dir + manifest produced by `ableton_render`, run `src/hallucinote/audio/` (loudness, master-bus contribution attribution, reverb verification) against the WAVs, write `MixReport` JSON to `songs/<slug>/analysis/<iso-ts>.json`. Scheduled as audio-analysis MVP Chunk 3 — see `.prawduct/artifacts/build-plan.md` and `.prawduct/artifacts/research-spike-audio-analysis.md`. **Verifiable signal:** `ableton_analysis` registered as an MCP tool with at least `analyze` + `get_latest_report` actions. (originally deferred 2026-05-16; Chunk 2 closed 2026-05-27)

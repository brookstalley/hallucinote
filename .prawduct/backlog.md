...
- **Audio-pipeline cumulative-Critic cleanups (deferred from the offline-browser-cache PR — pre-existing on develop, out of that PR's scope).** Findings the cumulative Critic surfaced against `main`; live in the #99/#100 audio code on develop. (a) **Drop unused `librosa>=0.10` core dep** in `pyproject.toml` (or move behind an extra) — per-band attribution shipped via `scipy.signal.butter`/`sosfiltfilt`; `git grep "import librosa" develop` returns nothing, yet it drags numba/soxr onto every install. **Signal:** `librosa` absent from core deps or gated behind an extra. (c) **Propagate the stdlib-only reversal** to the decision record at `.prawduct/project-state.yaml:258` ("Python 3.10+ stdlib-only runtime") — the branch added six core deps; `change-log.md` justifies it but the decision record wasn't updated. **Signal:** project-state.yaml decision reflects the dep adoption. _((b) dead `OvershootWindow` import shipped on feature/variable-tempo-windowing 2026-05-29.)_ (cumulative Critic, offline-cache PR 2026-05-28)

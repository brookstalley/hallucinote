...
- **`remove_send` silently drops composer RT60 intent.** `sends.intended_rt60_s` is the first piece of pure composer authorship that lives on a row a pull-side `remove_send` can delete (chunks/notes/devices are V1 Ableton-authoritative; sends were too, until this column landed). Consistent with the project's "Live is authoritative on sends" policy, but worth either (a) a docstring on `remove_send` calling out the intent-loss, (b) refusing the delete when `intended_rt60_s IS NOT NULL` unless a `--drop-intent` flag is set, or (c) emitting a separate `SEND_INTENT_CLEARED` event when the cascade runs. **Verifiable signal:** decision recorded in a `decisions/` file OR `remove_send` docstring/behavior reflects the choice. **Sized:** small. (cumulative Critic note, audio-analysis follow-on 2026-05-28)

- **`analyze_handler` opens the song DB twice.** `_verify_song_db_exists` opens + closes for slug validation; `_collect_declared_sends` re-opens to read intent. Two opens are cheap on local SQLite but inelegant. Collapse: have `_verify_song_db_exists` return the open conn (or a `(conn, song_id)` tuple), let `_collect_declared_sends` reuse it, close once in `analyze_handler`. **Verifiable signal:** `analyze_handler` calls `init_db(...)` once per invocation, not twice. **Sized:** small. (cumulative Critic note, audio-analysis follow-on 2026-05-28)

- **Hypothesis property test on `set_send_intended_rt60` validator.** Project enables hypothesis (`tests/conftest.py` profiles). A one-liner over positive floats + None would harden the contract beyond the hand-picked boundary cases (`0.0`, `-0.5`) shipped today. **Verifiable signal:** `tests/unit/sync/test_mix.py` carries a `@given(...)` test for the validator. **Sized:** tiny. (cumulative Critic note, audio-analysis follow-on 2026-05-28)

...
- **Variable-tempo-accurate beat→sample windowing for section + overshoot analysis.** Section-windowed loudness AND per-section per-band contribution attribution now ship (`MixReport.per_section` with `SectionMetrics.attribution` — `attribution.band_attribution` ranks per-band stem RMS share per section, answering "kick + bass dominate the chorus low end"). The remaining gap: section windowing (`analyze._measure_sections` via `intersect_window`) and overshoot rebeat-ing (`analyze._rebeat_overshoot`) share a CONSTANT-tempo linear beat↔sample map (`beats_per_second = span_beats / duration_s`). Songs with a multi-entry `tempo_map` get approximate sample boundaries for both section windows and overshoot beat positions. Walk the `tempo_map` instead. **Verifiable signal:** `_rebeat_overshoot` / `intersect_window` derive sample bounds from the `tempo_map`, not a single `beats_per_second`. **Sized:** medium. (Section-scoped masking analyzer is tracked separately in P2.) (loudness + per-section attribution shipped 2026-05-28; variable-tempo carried forward from spike §9)
...
- **Audio-pipeline cumulative-Critic cleanups (deferred from the offline-browser-cache PR — pre-existing on develop, out of that PR's scope).** Three findings the cumulative Critic surfaced against `main`; all live in the #99/#100 audio code on develop, not in the offline-cache diff. (a) **Drop unused `librosa>=0.10` core dep** in `pyproject.toml` (or move behind an extra) — per-band attribution shipped via `scipy.signal.butter`/`sosfiltfilt`; `git grep "import librosa" develop` returns nothing, yet it drags numba/soxr onto every install. **Signal:** `librosa` absent from core deps or gated behind an extra. (b) **Remove dead `OvershootWindow` import** at `src/hallucinote/audio/analyze.py:29` — unused in the module body. **Route to `feature/variable-tempo-windowing`**, which already edits `analyze.py` (avoids a merge conflict). **Signal:** import gone. (c) **Propagate the stdlib-only reversal** to the decision record at `.prawduct/project-state.yaml:258` ("Python 3.10+ stdlib-only runtime") — the branch added six core deps; `change-log.md` justifies it but the decision record wasn't updated. **Signal:** project-state.yaml decision reflects the dep adoption. (cumulative Critic, offline-cache PR 2026-05-28)

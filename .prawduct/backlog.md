...
- **Audio-analysis MVP Chunk 2 close-out — Section F feature emitter + master-strip in-Live + multi-analyzer verification + /critic.** Sub-chunk 2B's recording-path half shipped 2026-05-26 (+0.31 audio buffer drift on transport-position-sync render — well inside ±4 buffer GO criterion). Remaining work to fully close Chunk 2: (a) **Section F feature emitter** — audio tap → K-weighted LUFS + sample peak + low-mid band → `[pack s f f f]` → `[udpsend 127.0.0.1 11201]` at 30 Hz to the sidecar. Independent of recording path; doesn't gate it. Per AUTHORING-CHUNK-2B.md Section F. (b) **Master-strip in-Live verification** — Python `master=True` path is shipped + unit-tested (sub-chunk 2A); needs to be exercised by loading the analyzer onto the master strip via Live's UI or MCP, then confirming `ableton_render` produces a master.wav. (c) **Multi-analyzer simultaneous capture** — drop the analyzer on multiple tracks + master, run `ableton_render`, confirm N+R+1 WAVs in `captures/<ts>/` with shared start/stop beats (transport-sync guarantees per-sample alignment). (d) **PDC cross-correlation** — pick any track WAV vs master WAV, compute cross-correlation peak lag, confirm ≤ 64 samples (audio-analysis spike §1 requirement). (e) **`/critic chunk`** — review the cumulative Chunk 2 work. (f) Backlog cleanup — delete this entry. **Verifiable signal:** `manifest.json` from a real `ableton_render` call contains entries for ≥2 tracks + master with `frames_received` > 0 each AND cross-correlation analysis runs. **Sized:** medium (mostly in-Live verification + bug-find loops; the architectural hard part landed today). (audio-analysis MVP Chunk 2B partial 2026-05-26)

## P1 — Strong benefit, moderate effort

- **Section-windowed audio analysis.** Same MixReport metrics (loudness, masking, contribution attribution) scoped to chorus / verse / bridge using the song's clip schedule, not just full-song aggregates. Highest-leverage next step per audio-analysis spike §9 — answers "is the chorus actually louder than the verse?" and "did the bass-cut help in the section it was supposed to." Requires reading `cue_points` + arrangement clip schedule and slicing rendered WAVs accordingly. **Verifiable signal:** `MixReport.per_section` field populated with metrics keyed to cue-point section names. (spike §9 defer 2026-05-23)
...
- **Compose-time built-in content selection by name for portability-strict mode.** Today picking a drum kit / instrument preset by name requires Live to be running (FileIds are per-machine — Case A). Strict-portability snapshot-author-time references like `"preset_query": "Drums/Kit-Core 909"` already resolve at push time on the push side (`push.py:1084-1099`). The remaining gap is the snapshot-author-time UX: a way to express kit picks offline without machine-specific FileIds, without the awkward push-then-recapture loop. Strict-mode-only (loader refuses on unresolvable queries, no fuzzy matching). **Verifiable signal:** `/song-pick-instruments` (or a sibling) accepts a portability-strict mode that writes only `preset_query` references. **Sized:** small once browser search lands (it has, per Arc 7-tail E3 + W13-A closeout). (neon-feedback test session 2026-05-20)

- **Audio analysis (`ableton_analysis`) — the offline analysis pipeline atop captures.** `ableton_render` shipped in audio-analysis MVP Chunk 2 sub-chunk 2A (2026-05-26). `ableton_analysis` is the remaining half — consume the captures dir + manifest produced by `ableton_render`, run `src/hallucinote/audio/` (loudness, master-bus contribution attribution, reverb verification) against the WAVs, write `MixReport` JSON to `songs/<slug>/analysis/<iso-ts>.json`. Scheduled as audio-analysis MVP Chunk 3 — see `.prawduct/artifacts/build-plan.md` and `.prawduct/artifacts/research-spike-audio-analysis.md`. **Verifiable signal:** `ableton_analysis` registered as an MCP tool with at least `analyze` + `get_latest_report` actions. (originally deferred 2026-05-16; render half closed 2026-05-26)

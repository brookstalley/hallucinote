...
- **Cross-resolver lock-test for `resolve_query` ⇄ `_resolve_preset_query` semantics.** The offline matcher `preset_query.name_matches` and the walk depth `MIN_WALK_DEPTH` are pinned to the MCP push-time side by lock-tests (`tests/unit/sync/test_compat.py`), but `preset_query.resolve_query`'s strict-single-match / `path_prefix` / 2+-ambiguity-with-paths semantics are a hand-maintained mirror of `hallucinote_mcp.handlers.device._resolve_preset_query` with NO cross-check. Verified faithful as of 2026-05-28, but future drift in the push resolver's ambiguity/prefix handling would trip no test. Build a shared-fixture lock-test: a small fake browser tree + its flattened-inventory equivalent, run both resolvers over the same queries (single-match, 0-match, 2+-match, path_prefix hit/miss), assert identical outcomes. **Verifiable signal:** a test in `test_compat.py` drives both resolvers over a shared fixture and asserts agreement. **Sized:** small. (Critic note, offline-browser-cache 2026-05-28)

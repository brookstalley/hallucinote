...
- `apply_push_results` dispatch hardening (chunk-3 expansion of an existing chunk-2 backlog item). With chunk 3 the planner emits 9 new `key:` kinds — `track_volume:`, `track_pan:`, `track_mute:`, `track_solo:`, `track_arm:`, `track_color:`, `master_volume:`, `master_pan:`, and `send:` — all silently dropped today. `test_push_mix.test_apply_push_results_silently_drops_unknown_chunk3_keys` documents the gap. Switch to a raise-on-unknown dispatch table; folds into the chunk-2 silent-ignore item. (critic, chunk 3)

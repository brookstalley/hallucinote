...
- **[ARR-1H9C]** Harmony/key as a first-class structural axis (the biggest arrangement-model gap)
  `effort: L · impact: L · area: arrangement · source: review · added: 2026-05-30 · status: open · related: ARR-4V7P`

  Surfaced by a counter-example review of the arrangement model (`arrangement-model.md`). The model carries ONE structural curve — energy — but functional-tonal music (sonata form, jazz changes, 12-bar blues, most pop) is driven by HARMONIC tension/resolution: tonic-vs-dominant established and resolved, modulation, the recap landing home. The model can author the notes but cannot carry "this section is in the dominant, resolved to tonic at the recap" AS STRUCTURE — it's blind to the thing doing the dramatic work. This upgrades the jazz-"changes" open question into a real DECISION: **is harmony a modeled substrate, or does it stay in the notes?** Ruler-consistent lean (not a commitment): a harmonic AXIS co-equal to energy — sections carry key / mode / harmonic function (tonic/dominant/…) and optionally a chord progression; the composer still authors every note (a harmony GENERATOR would be a stamp), but the model carries the harmonic narrative, makes the variation ops key-aware (`transpose` shifts pitch but is key-blind today), expresses modulation + recap-resolution, and feeds a read-side lens that verifies the notes match the declared harmony (like the masking analyzer verifies mix intent). Grain is part of the decision (key/mode per section? Roman-numeral function? full changes?). Couples with ARR-4V7P and the DB-promotion phase. **Verifiable signal:** sections can carry a harmonic attribute (key/mode/function) AND `arrangement-model.md` records the substrate-vs-notes decision with rationale. **Sized:** large. (arrangement-model counter-example review, 2026-05-30)
...
Closed investigations — no fix possible / structural-close on Ableton's roadmap. Kept for search so a future scrub doesn't re-open them without new evidence. Status `dropped` = investigated and intentionally not pursued.

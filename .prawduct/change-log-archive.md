...
<!-- Entries rolled out of `.prawduct/change-log.md` to keep that file inside this
     repo's `oversized_file_threshold_kb`. Verbatim and unedited; this file is
     history for humans and is read by no gate — `lib/change_log.py` names
     `.prawduct/change-log.md` specifically, so a tag here is inert.

     Every entry below already carries a `release=` key. Nothing release-pending
     is ever moved here: the ABSENCE of `release=` is what marks an entry pending,
     and archiving one would drop its scope out of the release gate silently. -->

Covers 2026-05-17 through 2026-08-12, newest first.
The live log is [`change-log.md`](change-log.md).

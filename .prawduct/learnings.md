...
## A realtime / long-playback MCP action needs a read-timeout policy entry, or verification is severed at scale

**Any MCP action that holds its wire response open for realtime work (transport playback, a long load, a render) MUST be registered in the server's read-timeout policy — otherwise the default socket timeout fires mid-operation, gets classed as a connection error, and discards the action's OWN result. The action's internal wall-clock deadline does not protect the wire; the wire has its own clock.**

ENV-9P4T chunk 01's `perform_batch` plays the transport over the union span of all changed arcs in record (minutes at mix scale) and returns each arc's `automation_state` — the ONLY verification a write-only surface has. It shipped without a `_READ_TIMEOUTS` entry, so `_read_timeout_for` returned the 15s default; a `socket.timeout` (an `OSError`) would fire mid-record, `push_execute` would class it as `connection_lost`, halt, and throw away the per-arc verification while Live kept recording. The bug was invisible at the scales the unit tests and the dev smoke driver used: the smoke driver hard-coded a 180s CLIENT read timeout (hiding the SERVER-policy gap), and the planner's own arcs were short (~8s < 15s). It only bites at the scale the whole feature exists for — the cumulative Critic caught it by reasoning about the union-span duration vs the timeout, not by running it. The single-arc `perform` it superseded had the same latent gap; nobody noticed because nobody performed a span longer than 15s.

**How to apply.** (1) When adding or renaming an MCP action whose handler sleeps/polls/plays for more than a couple seconds, add `(tool, action)` to `server.py`'s `_READ_TIMEOUTS` in the SAME change — `None` (unbounded) when the duration is content-dependent and the handler self-bounds (render, perform_batch), a generous bounded ceiling when it's long-but-finite (ensure_loaded). Add a regression test asserting the policy (`_read_timeout_for(...) is None`). (2) Don't let a test/dev harness's client-side timeout override mask a missing server-side policy entry — they're different layers; the real push path uses the server policy. (3) Treat "this action's wall-clock is content-proportional" as the trigger to check the read-timeout policy, the same reflex as checking the gesture-restore `finally`.

...
- **Mid-song tempo / time-signature changes don't reach Live.** You can author them — the DB records the song's true tempo and meter maps — but Live 12's MCP exposes tempo and signature only as single global values, so push sets the bar-1 row and reports loudly that the rest were skipped (a real MCP gap, never silent data loss).

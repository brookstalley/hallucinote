...
| # | Question | Verdict | Evidence |
|---|---|---|---|
| 28 | **Does a hand-dropped sample survive `capture` → replay → push?** | **YES — CONFIRMED in two halves, not as one continuous run** | `capture_cli execute` wrote the hand-dropped `~/Downloads/bargain_not.wav` into `captured_session.json` under the Simpler's track. After `replay_capture`, the DB holds `devices.audio_file = '~/Downloads/bargain_not.wav'` on that device (`kind` `Simpler`, `display_name` `bargain_not`) — the capture→snapshot→replay half. The snapshot→push half is rows 21-22: push emits `assign_sample` from a `devices.audio_file` row and Live reads the path back. The two were verified separately because the link state blocked a continuous push (row 29) |

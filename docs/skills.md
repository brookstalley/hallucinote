...
| Skill | What it does |
|---|---|
| `/hallucinote:song-brief` | **The elicitation stage — runs before `/song-new`.** Sweeps the load-bearing dimensions a starting prompt left open (harmony, tempo, production stance, named narrative turns, meter, the section time budget, and the mechanism behind every named gesture), marking each **DECIDED / UNDECIDED / NOT-APPLICABLE**, then **asks the composer openly** about the ones that are theirs (identity: what it should do to someone, whether anyone is singing, the harmonic world, what it sounds like, the shape) while deciding and showing the ones that are yours (craft: BPM, meter arithmetic, voicings, mix moves) — never a questionnaire, and silent when nothing is open. Writes `annotations/01-the-brief.md`, which supplies the tempo/meter/section values `/song-new` needs as arguments. |

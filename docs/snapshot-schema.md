...
- `class_name` (optional) is Live's **internal class identifier** — `"Compressor2"`, `"PhaserNew"`, `"DrumGroupDevice"`, `"PluginDevice"`, etc. Informational; drives plugin discrimination (compat-check checks this for the third-party-plugin family `{PluginDevice, AuPluginDevice, Vst3PluginDevice}`). Capture-from-Live populates it; hand-authored snapshots may omit it.

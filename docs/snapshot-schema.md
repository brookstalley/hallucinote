...
1. **Captured** — `"$PY" -m hallucinote.cli capture` (with `$PY` the plugin-env interpreter — see [`running-the-engine.md`](running-the-engine.md)) walks a running Live set via MCP and writes the result. Use this once you've staged the target Live shape.
...
- `master.devices` (optional, **SNP-4K7M**) — a device chain on the master, same shape as a track's `devices[]` (see "Device chain shape" below). A master Limiter / EQ declared here round-trips: `replay_capture` materializes it on the master chain, the push side loads it (DEV-6M2K), and `/song-snapshot` captures a hand-placed master device back. The `HallucinoteAnalyzer` is filtered out at capture/migration exactly as on tracks and returns. Master **automation envelopes** are a separate surface, but not a closed one: master mixer (`mixer_volume` / `mixer_pan`) and master-chain `device_parameter` envelopes ARE authorable and pushable today — `classify_envelope_route` routes a master host to `perform`, and push's `performed_automation` phase gesture-records the arc into arrangement automation (ENV-7G4K, ENV-9P4T). What **MAW-4K7P** still covers is *fidelity, not authorability*: the perform route is realtime gesture-recording (scheduling-bound ~2.5 Hz, no curve kinds, write-only — no LOM read-back), so a truly lossless master ride still needs the unbuilt `.als` XML write path. Two narrower limits are permanent Live facts, not MAW-4K7P: `send_level` on the master is unauthorable (Live's master has no sends), and a master device must be linked (`probe-and-link`) before its arc can perform. See `docs/song-authoring-conventions.md` → "Master, group, and return envelopes — performed automation" for the authoring contract, and "The PRE-MAIN submaster bus" for why the convention is that nothing rides the master by default.
...
- `guess_uri` (optional) — Live's `preset_uri` for browser-reload. Most devices loaded via `ableton_browser` carry a URI; default empty devices may not. **Per-machine** (FileIds differ across machines for the same preset). Captured automatically by the capture CLI (`"$PY" -m hallucinote.cli capture`); **hand-authoring URIs is unreliable** — prefer `preset_query` (below) for portable compose-time selection, OR probe via `ableton_browser` first.
- `preset_query` (optional, mutually exclusive with `guess_uri`) — **cross-machine portable preset selector.** A JSON object `{root, pattern, mode?, path_prefix?, case_sensitive?}` resolved at push time on the consumer's machine via `ableton_browser(action='search')`. The push planner threads this into `ableton_device(action='load', preset_query=...)`, which refuses the load if 0 or 2+ matches (strict — no fuzzy match).
  - **`root` must be one of** (loader-accepted enum, NOT the resource-URI form): `instruments`, `audio_effects`, `midi_effects`, `drums`, `plugins`, `samples`, `user_library`, `packs`. Common typo: writing `effects` (the resource-URI form) instead of `audio_effects` — the loader refuses. Compat-check (`"$PY" -m hallucinote.cli compat check <slug>`) catches this at compose time.
...
4. **Recapture via `/hallucinote:song-snapshot` (or `"$PY" -m hallucinote.cli capture`).** Writes a `captured_session.refresh.json` side-by-side; diff against the existing snapshot; confirm; overwrite. Now `captured_session.json` reflects the actual chain state — the next push from a fresh DB will reproduce it exactly.
...
2. **Capture-then-recapture loop.** Stand up the device by hand in Live (or via `ableton_device(action='load')` directly), then run `"$PY" -m hallucinote.cli capture` against the running set. The capture pipeline records `guess_uri` for you — accurate, but per-machine.
...
- **Synthetic snapshots are fine as a starting point**, but their `volume`/`panning` defaults won't match the eventual Live state. Recapture (via `"$PY" -m hallucinote.cli capture`) once you've staged Live.

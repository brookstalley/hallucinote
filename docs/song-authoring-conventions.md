...
- **It refuses rather than half-doing it.** Two states stop the command *before the first delete*, each naming what it found. A device in the affected span with no loadable identity — a `placeholder` (an intentionally empty slot, which a tail-append cannot reproduce mid-chain), or a row with no kind or preset selector. And a device Live reports in the chain that the rebuild can neither address nor delete: anything it leaves behind survives the demolish and pushes every reloaded device one slot further down than the journal says, so the restore would write real dialed values onto the wrong device. The `HallucinoteAnalyzer` is the one device allowed to survive a rebuild — the render path owns it and puts it back itself. Deleting something that cannot be put back is unrecoverable, so both checks run while the chain is still there to look at.

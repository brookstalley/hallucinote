...
4/4 default; flag and confirm anything else. Critical when the song wants meter changes — declare them on the arrangement (`Arrangement(meter=…)` / `meter_change(at_bar=…)` / `section(..., meter=…)`) and every position resolves through the song's meter map. Only the bar-1 meter reaches Live's RULER, which costs bar numbering and the metronome, not playback (see `docs/song-authoring-conventions.md` → *Meter (4/4 vs. other)*). **The agent needs to know early whether to attempt non-4/4** (generators are 4/4-shaped within bars; non-4/4 sections need hand-authored patterns).

...
- **A3 — Drum Rack pad-mapping discovery.** M1-C shipped the `pad_info` MCP action + `drum_pad_mappings` schema + `replace_drum_pad_mappings` mutator + `get_drum_pad_mappings` query + the `Kit` class (`generators/kit.py`). What's **missing**: the auto-population path (devices phase probes `pad_info` after a Drum Rack load and persists the result) and stronger Kit-side resolution semantics so the Hot Rod Kit wrong-sound case (GM-default-collides-with-differently-named-chain) fails loudly instead of silently substituting. **Shipped shape:** extend `Kit` with `try_pitch_of(canonical) -> int | None` (None on populated-but-no-match) and `assert_has(*canonicals)` (bulk fail-fast); upgrade `pitch_of` to raise on the wrong-sound case with a teaching message naming the colliding chain. Auto-population via a `push_execute` walker over `Q.get_linked_drum_racks_for_session` after the devices phase. Closes the Hot Rod Kit cautionary tale (cowbell-on-metal). This was the substantive chunk.
...
- **B2 — annotations MCP surface.** `ableton_annotation` tool with actions `list`, `add`, `update`, `delete`, `get_at_bar` wrapping the W8-C mutators/queries. `hallucinote://annotations/<song_id>` resource for cheap full-song reads. **Session-briefing wiring excluded** per the out-of-scope note above. Without this surface, W8-C's annotations table is storage without affordance — agents can't read or write annotations during composition.
- **B3 residual — missing provenance columns.** Add `prompt_text` (TEXT, nullable), `parent_id` (INTEGER self-FK, nullable), `metadata_json` (JSON, nullable — `{model, git_sha, branch, session_id, hostname}`) to `requests`. Extend `M.create_request` + `M.request(...)` context manager to accept these on open. Existing W8-B columns (`kind`, `duration_ms`, `outcome`) untouched.
- **B4 — wire provenance into push/pull/capture/compose drivers.** Push/pull skills open `kind='push'`/`kind='pull'` request before fan-out; close after `apply_*_results`. Capture opens `kind='capture'` with the Live-set path. Compose sessions open `kind='compose'` at session start with the user's initial prompt as `prompt_text`. MCP per-tool dispatcher opens an inferred `kind='mutate'` request if no parent exists (degraded but always-present provenance).

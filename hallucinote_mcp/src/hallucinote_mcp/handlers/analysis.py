...
try:
    from hallucinote.audio import analyze_mix
...
def _verify_song_db_exists(song_slug: str) -> None:
    ...
    Open + close — we don't yet read intent (MVP DB has no
    reverb_send_intent schema), but we want to fail loud if the slug is
    a typo rather than silently writing a MixReport against random
    captures.
    """
    db_path = resolve_db_path(song_slug)
...
def analyze_handler(
    ...
    *,
    ...
    report = analyze_mix(captures_path)

...
def _start_new_song(
    ...
    text = (
        ...
        f"{intent_clause}\n\n"
        f"Steps:\n\n"
        f"0. **Pre-composition elicitation** (`docs/new-song-checklist.md` "
        f"— the authoritative 14-item must/should/emergent reference). "
        f"Read what the user said; infer everything you can; **state your "
        f"inferences explicitly** so the user can correct you cheaply. "
        f"Ask 2-3 targeted questions for must-haves you genuinely can't "
        f"infer (intent/purpose, genre anchor, length/structure, "
        f"vocals?, instrumentation). Default the should-haves "
        f"(tempo/feel, time-sig, harmonic strategy, production style, "
        f"arrangement arc) with 'I'll go with N — say if you want "
        f"different.' Let nice-to-haves (references, hard constraints, "
        f"hooks, density) emerge unless the user volunteers. Guidance, "
        f"not script — freeform exploration is allowed.\n\n"
        ...
        f"1a. **Persist decisions from Step 0** to "
        f"`songs/{slug}/decisions/NN-<topic>.md` — one file per "
        f"non-trivial decision. Number prefix for ordering. Format: the "
        f"question, the answer, who decided (user / inferred / "
        f"agreed-after-confirm), the rationale. Future sessions read "
        f"these via `/song-context` so the song's intent survives "
        f"`/clear`.\n\n"
        ...
        f"`pick_instruments_for_song` prompt to translate Step 0's "
        f"instrumentation description into real device picks. Default to "
        f"`portability='strict'` unless the user signaled tolerance for "
        f"third-party plugins or the style hint demands something stock "
        f"devices can't reach. For built-in Live content (drum kits, "
        f"instrument presets), prefer Sweep B's `preset_query` in the "
        f"snapshot ({{root, pattern}}) — portable across machines. For "
        f"third-party plugins, use `preset_uri` (per-machine). Then "
        f"either push + load-and-recapture or hand-author the snapshot "
        f"with preset_query and proceed.\n\n"

...
def test_preflight_report_package_block_includes_version_and_root():
    ...
    assert "remote_script_exclude" in report["package"]
    # Exclude list is the load-bearing safety net — skill body relies on it.
    assert "server.py" in report["package"]["remote_script_exclude"]

...
- **`remote_script.candidates[*]`** — `installed: true` with `matches_mcp_server: false` is a stale install → this run is an **update** (Step 3 replaces it). `matches_mcp_server: true` → the wire shape is current; you may skip to Step 4 **unless `matches_vendored_content` is `false`** (next bullet — the handshake covers only the wire-shape files). (When `server.confirmed: true` this match is against the **running** server, so a `true` here genuinely means the next handshake succeeds; when `false`, treat it as advisory.)
- **`remote_script.candidates[*].matches_vendored_content`** — the *advisory* half, and it is a **recommendation, never a refusal**: `false` means the vendored tree differs from what this source would ship even where the handshake doesn't look. With `matches_mcp_server: true` this is the case worth saying out loud — the handshake will pass and Live is running **stale non-wire code**. Read `differing_paths` (capped; `differing_count` is exact) before you speak: `analyzer/*` is code Live *executes* during a render, so say so and offer the Step 3 re-vendor with `--force`; `server_side/*` is imported in Live but never run there, so a difference confined to it is noise — mention it, don't push. `null` means the comparison did not run — check **`content_read_error`** beside it before you call that benign: `null` with no `content_read_error` is simply "no install"; `installed_tree_unreadable` means a Remote Script IS installed but could not be read, so the advisory is blind and Live may be running stale code — say that plainly and offer the Step 3 `--force` re-vendor rather than reporting nothing to compare. `source_tree_unreadable` is the mirror: the installed copy read fine but THIS checkout did not, so the fault is the source side and re-vendoring from it would ship whatever could not be read — investigate the checkout first.

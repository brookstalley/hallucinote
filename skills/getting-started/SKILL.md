...
**Before any song work, run the workspace check once:** `"$PY" -m hallucinote.cli init-workspace --check`. Its JSON carries two **independent** keys, and each is read on its own below — `already_workspace` decides which bullet you're in; `governed_repo` fires whatever that answer was.
...
**`governed_repo` is true — whichever bullet you took.** This directory is at or inside a Prawduct-governed engineering repo, and song work started here inherits an engineering register: governance advisories and status footers landing in the middle of a creative conversation. Say so once, in a sentence, before any song work — *"heads up, this is a governed engineering repo, so you may see governance chatter around the music; a songs workspace of its own keeps the session about the song"* — and carry on unless they redirect you. It's a **heads-up, never a block**, and it does not depend on `already_workspace`: a governed repo can legitimately hold a songs workspace (this one ships the `examples/` demo workspace), so both flags are true at once there and the heads-up still applies.

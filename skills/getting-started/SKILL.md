...
- **Set up and in a workspace** → offer the fork: *"Want to start a new song, or open one you already have?"* — new → `/song-brief` (the elicitation stage: it turns their prompt into the brief, and produces the tempo/meter/section values `/song-new` needs), then `/song-new`; existing → name it and push it. The full lifecycle map is `/song-workflow`.

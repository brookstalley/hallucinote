...
> **Running engine commands.** The engine ships in the plugin's uv env. Resolve `$PY` once from `ableton://server/info`'s `python`; the `hallucinote pull …` commands below run as `"$PY" -m hallucinote.cli pull …`. See [`docs/running-the-engine.md`](../../docs/running-the-engine.md).
...
```bash
"$PY" -m hallucinote.cli pull execute device-parameters <session_id> \
  --song <slug> --dry-run
```

...
These are the `intent-collaboration-model.md` registers at stage 0. **Note** is
the one agents skip, and skipping it is what inflates observations into
proposals: *"if there are vocals, a 7/4 chorus is hard to sing over — I'd keep the
phrase inside 4 bars"* wants no answer. Say it and move on.
...
- **"Just go" ends the stage immediately.** Remaining identity rows get decided
  and *shown*, recorded in the brief with `agent` as the decider so they read as
  yours to correct.
...
Two open identity questions (2, 4). One **show** — the harmonic read, offered in
two sentences with an explicit exit, because harmony is identity and they hadn't
spoken to it. One **note**. Tempo, bar counts and meter arithmetic are craft:
decided, mentioned in passing, never put up for review.
...
|---|---|---|---|
| Vocals | DECIDED | instrumental; the lead guitar carries the vocal role | user |
| Harmony | DECIDED | D minor throughout; guitar B♮ vs brass B♭ is the argument | agent |
| Tempo | DECIDED | 132 BPM | agent |
| Alternate tuning | NOT-APPLICABLE | — | inferred |
| Outro octave drop | DECIDED | master Shifter, `Pitch Coarse` 0 → −12, pre-limiter | agent |
...
- **`Decided by` distinguishes `user` from `agent`.** Anything marked `agent` is
  something you chose on the composer's behalf — that column is how they find it
  and take it back.
...
- **Every identity dimension was either answered by the composer or asked about**
  — or, where they said "just go", decided and marked `agent` so they can take it
  back.

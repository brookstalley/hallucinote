...
- **Within-song meter changes ARE recordable** — `M.add_time_signature_point` takes one row per change, at the bar it starts on. The `--signature` argument here is the song's *primary* meter (it becomes the bar-1 row); author the rest in `build.py`. Only the bar-1 row reaches Live (Live 12.4's MCP has no `song_signature` automation target) and push warns about the rest, so realize the other meters as felt groove over the single ruler — never ask the user to accommodate that. See `docs/song-authoring-conventions.md` → *Meter*.

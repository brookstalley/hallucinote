...
def build(reset: bool = False) -> str:
    ...
    `--reset` is a soft reset (W18-C): wipes rebuild-by-build.py content but
    preserves the mix layout (tracks, returns, devices) AND the Ableton
    projection (`ableton_sessions` + `ableton_links`). Delete the DB file
    directly for a full clean slate.
    """
    conn = init_db(DB_PATH)
    try:
        if reset:
            song = Q.get_song_by_name(conn, "neon-feedback")
            if song is not None:

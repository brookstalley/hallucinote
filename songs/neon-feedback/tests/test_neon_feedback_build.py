"""Shape test for neon-feedback's build.py.
...
def build_module(tmp_path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("neon_feedback_build", BUILD_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    monkeypatch.setattr(module, "DB_PATH", tmp_path / "neon-feedback.db")
    return module
...
def test_build_runs_clean_and_produces_shape(build_module):
    ...
    try:
        ...
        assert section_names == ['intro', 'verse-1', 'pre-chorus-1', 'chorus-1', 'verse-2', 'pre-chorus-2', 'bridge', 'bridge-twist', 'bridge-return', 'chorus-2', 'outro']
        ...
        assert len(tempo) >= 1
        assert tempo[0]["tempo_bpm"] == 120.0

...
user customization — means the tracks are no longer recognizable defaults, and
each caller falls back to treating them as authored content rather than
guessing.
"""
...
Live displays. The probe-and-link returns matcher strips that prefix and
matches against the DB's stripped form (W4-C); cleanup keys off the raw names,
because cleanup is about deleting Live-side defaults the song has not claimed,
not about matching by stripped name."""

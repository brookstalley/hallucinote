...
def test_kick_stumble_shape():
    notes = drums.kick_stumble(4, kit=_KIT)
...
def test_lazy_snare_lay_back():
    notes = drums.lazy_snare(2, kit=_KIT)
...
def test_trip_hop_hats_alternation_and_boost():
    notes = drums.trip_hop_hats(4, kit=_KIT, boost_bars=[3])
    assert all(_well_formed(n) for n in notes)
...
def test_trip_hop_pattern_composes():
    notes = drums.trip_hop_drum_pattern(4, kit=_KIT, fill_bars=[3])
...
def test_drum_generators_respect_start_beat():
    ...
    notes = drums.kick_stumble(2, kit=_KIT, start_beat=16.0)
    assert min(n["start_beats"] for n in notes) == 16.0
    # lazy_snare
    notes = drums.lazy_snare(2, kit=_KIT, start_beat=16.0)
    assert min(n["start_beats"] for n in notes) >= 16.0
    # trip_hop_hats
    notes = drums.trip_hop_hats(1, kit=_KIT, start_beat=8.0)
    assert min(n["start_beats"] for n in notes) == 8.0
    # bossa_shaker
    notes = drums.bossa_shaker(1, kit=_KIT, start_beat=12.0)
    assert min(n["start_beats"] for n in notes) == 12.0
    # tresillo_hats
    notes = drums.tresillo_hats(1, kit=_KIT, start_beat=20.0)
...
def test_trip_hop_drum_pattern_respects_start_beat_for_fills():
    ...
    notes = drums.trip_hop_drum_pattern(4, kit=_KIT, start_beat=16.0, fill_bars=[3])
...
def test_trip_hop_drum_pattern_handles_non_bar_aligned_start_beat():
    ...
    notes = drums.trip_hop_drum_pattern(4, kit=_KIT, start_beat=0.5, fill_bars=[3])
...
def test_ghost_drum_helpers_take_start_beat():
    ...
    kicks = drums.ghost_kicks([0], kit=_KIT, start_beat=7.0)
    assert kicks[0]["start_beats"] == 7.0 + 3.5  # 10.5
    snares = drums.ghost_snares([0], kit=_KIT, start_beat=7.0)
    assert snares[0]["start_beats"] == 7.0 + 3.75  # 10.75
    lifts = drums.open_hat_lifts([0], kit=_KIT, start_beat=7.0)
...
def test_kick_stumble_default_beats_per_bar_unchanged():
    ...
    notes = drums.kick_stumble(2, kit=_KIT)
...
def test_kick_stumble_non_default_beats_per_bar_scales_bar_starts():
    ...
    notes = drums.kick_stumble(2, kit=_KIT, beats_per_bar=3.5)
    downbeats = sorted(
...
def test_lazy_snare_beats_per_bar_scales_bar_starts():
    notes = drums.lazy_snare(2, kit=_KIT, beats_per_bar=6.0)  # e.g. 6/4
    starts = sorted(n["start_beats"] for n in notes)
...
def test_trip_hop_hats_beats_per_bar_scales_bar_starts():
    notes = drums.trip_hop_hats(2, kit=_KIT, beats_per_bar=3.0)
...
def test_bossa_shaker_beats_per_bar_scales_bar_starts():
    notes = drums.bossa_shaker(2, kit=_KIT, beats_per_bar=2.0)
...
def test_ghost_kicks_beats_per_bar_scales_bar_offset():
    notes = drums.ghost_kicks([0, 1], kit=_KIT, beats_per_bar=3.5)
...
def test_trip_hop_drum_pattern_threads_beats_per_bar_to_subprimitives():
    notes = drums.trip_hop_drum_pattern(
        2, kit=_KIT, fill_bars=[1], beats_per_bar=3.5,
    )
...
def test_feel_none_preserves_canonical_positions():
    ...
    baseline = drums.kick_stumble(2, kit=_KIT)
    feel_none = drums.kick_stumble(2, kit=_KIT, feel=None)
...
def test_kick_stumble_feel_only_shifts_matching_positions():
    ...
    notes = drums.kick_stumble(1, kit=_KIT, feel={1.5: -0.1})
    starts = sorted(n["start_beats"] for n in notes)
...
def test_lazy_snare_feel_stacks_with_lay_back():
    ...
    notes = drums.lazy_snare(1, kit=_KIT, feel={1.0: -0.05}, lay_back=0.04)
    starts = sorted(n["start_beats"] for n in notes)
...
def test_trip_hop_hats_swing_eighths_via_feel():
    ...
    notes = drums.trip_hop_hats(1, kit=_KIT, feel=swing)
    starts = sorted(n["start_beats"] for n in notes)
...
def test_per_part_feel_punk_drums_lazy_pluck_independent():
    ...
    drums_notes = drums.kick_stumble(1, kit=_KIT, feel=punk_feel)
...
def test_per_clip_feel_verse_vs_chorus_same_generator():
    ...
    verse = drums.kick_stumble(8, kit=_KIT, feel={2.75: -0.02})
    chorus = drums.kick_stumble(8, kit=_KIT, feel={2.75: +0.025})
...
def test_feel_threads_through_aggregate_trip_hop_drum_pattern():
    ...
    feel = {0.0: -0.01}
    notes = drums.trip_hop_drum_pattern(1, kit=_KIT, feel=feel)
